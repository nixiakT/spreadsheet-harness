...
- Extend an adjacent formula with `fill_formula`; it translates relative references more reliably than manually composing each formula. Use a full range like `H6:P9`; a single target cell like `P6` is treated as the endpoint `H6:P6`.
- Before filling formulas, lock any range that must remain fixed in the fill direction on both endpoints. For example, use `SUM($E6:$G6)` for a fixed actuals range, not `SUM(E6:G6)` or `SUM($E6:G6)`, because those drift or expand when filled right.
