...
# SheetCompass has no released runner/checkpoint.  This proxy uses the
# harness basic composition, whose structure/analysis/memory/verification
# skills implement the closest available Explorer->Programmer->Reflector loop.
exec .venv/bin/python -m spreadsheet_harness.cli benchmark v2-compare \
  ...
  --arm spreadsheet-harness-basic --output benchmarks/results/sheetcompass-graph-memory-proxy-qwen36-v2-30-20260905 \
