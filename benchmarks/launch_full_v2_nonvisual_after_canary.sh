...
# Queue the complete non-visual v2 study behind the currently running canary.
# The canary PID is intentionally explicit so this cannot race another provider run.
canary_pid="${1:?usage: launch_full_v2_nonvisual_after_canary.sh CANARY_PID_OR_0}"
if [[ "$canary_pid" != 0 ]]; then
...
exec .venv/bin/python -m spreadsheet_harness.cli benchmark v2-compare \
  ...
  --output benchmarks/results/spreadsheetbench-v2-qwen36-full-nonvisual-basic-fresh-v2-20260829 \
