...
typeset root="benchmarks/results/spreadsheetbench-v2-dashscope-qwen3-coder-plus-30-parallel-20260904"
mkdir -p "$root"
...
printf '%s\n' "${task_ids[@]}" | xargs -P "$parallelism" -I '{}' zsh -c '
  set -euo pipefail
  task="$1"
  slug="${task//\//_}"
  out="benchmarks/results/spreadsheetbench-v2-dashscope-qwen3-coder-plus-30-parallel-20260904/${slug}"
  log="benchmarks/results/spreadsheetbench-v2-dashscope-qwen3-coder-plus-30-parallel-20260904/${slug}.log"
  source /data/zju-160/tongzeyuan/.config/litellm/lab.env
  unset OPENAI_API_KEY HTTP_PROXY HTTPS_PROXY http_proxy https_proxy ALL_PROXY all_proxy
  cd /data/zju-160/tongzeyuan/spreadsheet-harness
...
echo "parallel task launch complete: ${#task_ids} tasks, ${parallelism} concurrent workers"
