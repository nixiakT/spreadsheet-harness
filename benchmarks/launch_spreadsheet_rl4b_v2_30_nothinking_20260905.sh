...
while IFS= read -r task_id; do task_args+=(--task-id "$task_id"); done < <(
  .venv/bin/python - <<'PY'
import json
x=json.load(open('benchmarks/results/spreadsheetbench-v2-glm52-nothinking-30-4arm-timeout-retry-20260904/manifest.json'))
...
exec .venv/bin/python -m spreadsheet_harness.cli benchmark v2-compare \
  ...
  --request-retries 1 --arm-order-seed 20260904 --base-url http://127.0.0.1:8626/v1 \
  --api-key-file /tmp/spreadsheet-harness-litellm.key --model Spreadsheet-RL-4B \
