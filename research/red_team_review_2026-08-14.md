...
| R6 | Multi-seed inference risked treating task-seed rows, or two selected model families, as independent. | Task is now the cluster; model families are unpooled fixed replication strata; single-seed Wilson/McNemar are restricted; Modification uses paired task-cluster intervals. The headline is a conjunction of one model-free mechanism gate and one primary-checkpoint real-task gate, each tested at level 0.05 with its own guardrail; the intersection--union decision requires both and gives no confirmatory status to secondary contrasts. | Freeze code, clustered power/precision analysis, non-inferiority margins, and exact decision rules before launch. |
