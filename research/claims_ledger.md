...
| --- | --- | --- | --- |
| C1 | Online enforcement reduces task-level accepted-terminal false completion without an unacceptable Accuracy or accepted-terminal coverage loss. | B6 vs B5 with identical online diagnostics; accepted-terminal false completion over all selected tasks plus preregistered Accuracy/accepted-terminal-coverage non-inferiority guardrails on the primary checkpoint and a separately reported second-family replication. First/any-attempt false completion remains diagnostic because capture precedes gating. | pending |
| C2 | Declaration elicitation and staged-containment enforcement have separately identified effects on wrong-target/off-target outcomes. | G1 vs G0 for elicitation and G2 vs G1 for enforcement on sealed Debugging-100, with gold-only masks, staged-containment oracle, grounding rejection/recovery, and a separately reported second-family replication. G2 vs G0 is descriptive total change, not one gate's effect. | pending |
| C3 | Workbook-enriched events catch bypasses that raw tool-call events miss. | Core same-policy enriched-vs-raw contrast within the same generic engine on sealed oracle attacks and valid controls; the full event/policy $2\times2$ is reported separately. | pending |
...
| C5 | For an accepted candidate, finalization lineage binds the exact scoring replica under a trusted host and storage. | FULL vs B6 plus the core model-free paired sealed finalization challenge, separate second-family replication, replay audit, field-corruption tests, accepted final/replica hashes, and all-row observer finalization records passing fresh audit. | pending |
